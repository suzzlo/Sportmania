class ChampionshipsController < ApplicationController
  ...
  def create
    ...
    @championship.user_id = current_user.id if current_user

    respond_to do |format|
      if @championship.save
        format.html { redirect_to @championship, notice: 'Championship was successfully created.' }
        format.json { render action: 'show', status: :created, location: @championship }
      else
  ...
  private
    ...
    def championship_params
      params.require(:championship).permit(:name, :start_date, :finish_date, :sport, :user_id)
    end
end
